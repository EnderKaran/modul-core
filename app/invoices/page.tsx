...
export default function InvoicesPage() {
  ...
  // Finansal Özet Hesaplamaları (Sadece ekrandaki filtrelenmiş veriye göre)
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-12 pt-8 relative print:p-0 print:m-0">
      
      {/* BAŞLIK VE KONTROLLER (Yazdırırken Gizlenir) */}
      <div className="flex justify-between items-end print:hidden">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-2">
          <h2 className="text-4xl font-black tracking-tightest text-slate-950 uppercase">Financial Ledger</h2>
          <p className="text-slate-600 text-base font-bold">Accounts payable and invoice settlement tracking.</p>
        </motion.div>
        ...
        <div className="flex gap-4">
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {isFilterOpen && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full right-0 mt-2 w-48 bg-white border-2 border-slate-950 shadow-2xl z-20 flex flex-col">
      ...
      {/* FİNANSAL ÖZET KARTI (Ekrandaki borcu gösterir) */}
      <div className="bg-slate-950 text-white p-8 border-2 border-slate-950 rounded-sm shadow-2xl flex items-center justify-between print:hidden">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">Total Outstanding Balance</p>
          <div className="flex items-center gap-3">
            <DollarSign className="w-8 h-8 text-emerald-400" />
            <h3 className="text-4xl font-black tracking-tightest">${totalOutstanding.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
          </div>
        </div>
        <div className="text-right">
          <p className="text-xs font-bold text-slate-400">Filtered Invoices: <span className="text-white">{filteredInvoices?.length || 0}</span></p>
        </div>
      </div>

      {/* ARAMA ÇUBUĞU (Yazdırırken Gizlenir) */}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white border-2 border-slate-300 rounded-sm shadow-2xl overflow-hidden relative print:border-none print:shadow-none">
        <div className="absolute top-0 left-0 w-full h-1.5 bg-slate-950 print:hidden" />
        ...
        <table className="w-full text-left border-collapse mt-1">
          <thead>
            <tr className="bg-slate-50 border-b-2 border-slate-300 print:bg-transparent print:border-slate-950">
              <th className="p-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 print:text-slate-950">Invoice No</th>
              <th className="p-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 print:text-slate-950">Supplier</th>
              <th className="p-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 print:text-slate-950">Linked Order</th>
              <th className="p-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 print:text-slate-950">Due Date</th>
              <th className="p-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 text-right print:text-slate-950">Amount</th>
              <th className="p-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 text-center print:text-slate-950">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y-2 divide-slate-100 print:divide-slate-300">
            {invoicesQuery.isLoading ? (
              <tr><td colSpan={6} className="p-12 text-center text-xs font-black uppercase tracking-widest text-slate-400">Loading Financial Records...</td></tr>
            ) : filteredInvoices?.length === 0 ? (
              <tr><td colSpan={6} className="p-12 text-center text-xs font-black uppercase tracking-widest text-slate-400">No invoices found.</td></tr>
            ) : filteredInvoices?.map((invoice: any) => (
              <tr key={invoice.id} className="group hover:bg-slate-50 transition-colors print:hover:bg-transparent">
                <td className="p-6">
                  <span className="text-sm font-mono font-black text-slate-950">{invoice.invoiceNumber}</span>
                </td>
                <td className="p-6 font-black text-slate-900 uppercase tracking-tight text-sm">{invoice.supplierName || 'Unknown Vendor'}</td>
                <td className="p-6 text-xs font-bold text-slate-500 uppercase tracking-widest">#{invoice.orderNumber || 'N/A'}</td>
                ...
                <td className="p-6 text-right">
                  <span className="text-sm font-black text-slate-950">${Number(invoice.amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </td>
                <td className="p-6 text-center">{renderStatusBadge(invoice.status)}</td>
              </tr>
            ))}
          </tbody>
        </table>
